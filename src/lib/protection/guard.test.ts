...
describe("guardPublicSubmission", () => {
  ...
  /*
    The other half of that policy, and the reason it is split.

    Outside production, no secret means the stub waves everything through so a
    fresh clone works. In production the same state means the bot gate has been
    off since deploy while the public layout told every visitor the form was
    protected — a permanent condition, not a wobble, so it refuses.

    This is the forged-submit case: a request arriving with no Turnstile token
    at a deployment that never configured Turnstile is exactly what an attacker
    sends, and exactly what this now rejects.
  */
  it("REFUSES in production when Turnstile was never configured", async () => {
    // vi.stubEnv, because NODE_ENV is readonly in the Next types and a bare
    // assignment does not compile. unstubAllEnvs puts it back.
    ...
    try {
      ...
      // The message must not accuse the reporter. It is our misconfiguration.
      if (result.allowed) throw new Error("expected a refusal");
      expect(result.message).toContain("on our side");
      expect(auditLines()).toContainEqual(
        expect.objectContaining({
          action: "bot_check_unavailable",
          outcome: "rejected",
  ...
  /*
    A REAL outage still fails open, and that is the deliberate asymmetry.

    A report not made is a report never made, and it may carry a 15-day clock
    that starts the day we receive it. The rate limiter has already run.
  */
  it("still ALLOWS in production when siteverify is merely unreachable", async () => {
    // vi.stubEnv, because NODE_ENV is readonly in the Next types and a bare
    // assignment does not compile. unstubAllEnvs puts it back.
    vi.stubEnv("NODE_ENV", "production");
